...
from E_Cart.settings import RAZORPAY_API_KEY,RAZORPAY_API_SECRET_KEY
import razorpay
...
def shopPage(Request,mc,sc,br):
    ...
    brands = Brand.objects.all()
    return render(Request,"shop.html",{'data':data,'maincategorys':maincategorys,'subcategorys':subcategorys,'brands':brands,'mc':mc,'sc':sc,'br':br})
...
def searchPage(Request):
    if(Request.method=="POST"):
        ...
        data = Product.objects.filter(Q(name__contains=search)|Q(color__contains=search)|Q(size__contains=search)|Q(stock__contains=search))
        maincategorys = Maincategory.objects.all()
        subcategorys = Subcategory.objects.all()
        brands = Brand.objects.all()
        return render(Request,"shop.html",{'data':data,'maincategorys':maincategorys,'subcategorys':subcategorys,'brands':brands,'mc':'All','sc':'All','br':'All'})
...
def placeOrderPage(Request):
    if(Request.method=="POST"):
        ...
        shipping = Request.session.get("shopping",0)
        ...
        if(mode=="COD"):
            subject = 'Your Order Has Benn Placed : Team E-Cart'
            message = """
                        Hello """+buyer.name+""" 
...
def contactPage(Request):
    if(Request.method=="POST"):
        ...
        c.save()
        messages.success(Request,"Thanks For Sharing Your Query With Us. Our Team Contact You Soon !!!")
    ...
    if(Request.method=="POST"):
        if(Request.POST.get("password")!=Request.POST.get("cpassword")):
            ...
            try:
                ...
                b.mobile = Request.POST.get("mobile")
                b.save()

                subject = 'Your Account Has Been Created : Team E-Cart'
                message = """
                        Hello """+b.name+""" Thanks
                        to Create Accout With Us
                        Now You Can Buy Latest Product.
...
def forgetPassword2(Request):
    if(Request.method=="POST"):
        ...
        try:
            ...
            messages.error(Request,"Un-Authorize User")
